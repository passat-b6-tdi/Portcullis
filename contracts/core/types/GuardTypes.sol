...
struct SettlementMessage {
    ...
    uint256 value; // settlement amount, in the token's own decimals
...
struct VolumeStat {
    uint256 baseline; // EMA of accepted value, normalised to 18 decimals
...
struct GuardState {
    ...
    // bounds (all value scalars are 18-decimal normalised)
    uint256 minValue;
    uint256 maxValue;
    // token => 10 ** (18 - tokenDecimals); 0 => token not allowed
    ...
    // rate limit (token bucket, 18-dec); capacity 0 => disabled
    uint256 rateCapacity;
    uint256 rateRefillPerSec;
    uint256 tokens; // allowance remaining as of lastRefill
    uint256 lastRefill;
    // volume policy: oracle set => delegate, else per-source EMA; both 0 => disabled
    IVolumeVerdictOracle volumeOracle;
    uint256 spikeFactorBps; // 30_000 => allow up to 3.0x baseline
    uint256 warmup; // no spike check until a source has >= warmup observations
    mapping(bytes32 => VolumeStat) volume; // srcId => rolling EMA
